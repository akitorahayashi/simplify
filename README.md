...
変更されたコードの品質を上げる Agent Skill を配布するプラグインです。レビュー範囲を再利用、
単純化、効率、抽象度の4観点でレビューし、意図された挙動を保ったまま改善をその場で
コードに適用します。
...
1. レビュー対象の取得 — PR の base、`origin/HEAD`、`main` の順でベースブランチを決め、
   ...
   どちらも空なら、ユーザーに確認のうえコードベース全体を対象にします。引数で PR 番号、
   ブランチ名、ファイルパスが渡された場合はそれを対象にします。
